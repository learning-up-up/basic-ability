...
Linux有两种常用的压缩格式，.tar和.gz。前者是对文件进行简单封装，后者可以极大的压缩文件体积

```tar [-c -v -x -f -z -C] 参数1（添加的）参数2（被添加的） ···```是压缩文件的基本命令

* -c：创建压缩文件
* -v：显示压缩和解压的进度
* -x：解压模式
* -f：要创建或要解压的文件，必须为与所有选项中的最后一个
* -z：gzip模式，否则就是最基本的tarball模式
* -C：选择解压的目的地

注意：-c选项和-x选项是冲突的。并且-C通常要单独使用

Linux也可以使用zip压缩格式。```zip [-r] 参数1 ···```，```unzip [-d] address```是解压命令

## 本地部署软件

对于apt中没有的软件包，我们安装时需要配置apt仓库。

### MySQL

下载仓库文件

配置仓库，使用dpkg命令的-i选项安装仓库

更新apt仓库信息：导入仓库的密钥信息，并使用```apt update```更新